...
function TeamForm(){
  ...
  const handleAddStack = (event: React.KeyboardEvent<HTMLInputElement>) => { 
    if(newStack === ''){
      return;
    };
    setTeam((prevTeam) => ({
  ...
  return (
    <form className='team_form' action="/post할URL" method="post">
      ...
      <div className='recruit_stack'>
        ...
        <input type="text" id='stack_search_input' value={newStack} onChange={handleStackInputChange} onKeyDown={(e) => handleAddStack(e)}/>
