...
function TeamForm(){
  ...
  const handleAddStack = (event: React.ChangeEvent<HTMLInputElement>) => {
    console.log(event);
    if(newStack === ''){
      return;
    };
  ...
  return (
    <form className='team_form' action="/post할URL" method="post">
      ...
      <div className='recruit_stack'>
        ...
        <input type="text" id='stack_search_input' value={newStack} onChange={handleStackInputChange}/>
        <button type="button" onClick={handleAddStack} onKeyDown={}>추가</button>
